...
- **2026-09-10 확인**: Figma 연결의 이미지 올리기(`upload_assets`) 주소는 `mcp.figma.com`이라 프록시에서 `connect_rejected` 403. 파일 간 이미지 이관에 필요하므로 Ken에게 "`mcp.figma.com`을 허용 목록에 추가해 주세요"라고 요청한 상태(2026-09-10). 추가 전에는 이미지는 자리표시로 두고 보고한다.
