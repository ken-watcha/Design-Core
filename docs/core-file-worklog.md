...
- **다음 할 일**: (승인 후) 본 파일 쓰기 단계 — Cover 텍스트·업데이트 로그, `🌊 플로우` 뼈대, SVOD·TVOD·검색 플로우 입구 Δ #13~15, 파일 이름·폴더 이동(사람), 색인 갱신. 그다음 설계안 3단계(기존 Core 갱신 자동화: 로그인/온보딩 파일럿 재현) 또는 4단계(Notion 요청 표 + Routine)
