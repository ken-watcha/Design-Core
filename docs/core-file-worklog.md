...
- **Ken 확인 1건(막힌 것)**: 진입점. Ken이 "고른 진입점 자체가 틀림"이라 했고, 브랜치 캔버스의 빨간 원 4개(`3489:21839` `3489:24943` `3489:24946` `3489:24948`)와 붙여 둔 360px 홈(`3489:23738`)이 **홈 최상단 피쳐링 셀(빅배너)과 그 아래 배너 띠**를 가리킨다 → 이 읽기가 맞는지 한 줄 확인 받으면 진입 섹션 마커 3개를 다시 찍고 Δ #13~#15도 다시 쓴다. **프로젝트 문서에는 진입 경로가 0건**(전 페이지 "진입/유입" 텍스트 없음)이라 문서로는 답이 안 나오는 항목
