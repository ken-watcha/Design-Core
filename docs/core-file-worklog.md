...
> 작성 2026-09-01 · 최종 갱신 2026-09-11 (세션 5) · 이 문서는 새 Claude 세션이 이어서 작업하기 위한 인수인계 기록
> 저장소 사본. 중계 지점은 아티팩트 https://claude.ai/code/artifact/72e9f09e-43fc-424e-ba39-fc4d80e303c5 (세션 마무리 시 둘 다 갱신)

## 다음 세션 시작점 (2026-09-11 세션 5 이후)

- **세션 5에서 끝낸 것**: ① `mcp.figma.com` 허용 후 **이미지 이관 재시험 성공** — `download_assets` → curl → `upload_assets` → POST 200, 자동 배치가 안 된 경우엔 응답 해시를 `use_figma`로 fills에 지정(항상 두는 안전장치). 파일 간 화면 옮기기는 **이미지까지 전부 자동**으로 확정(spike 문서·deep-clone.md·설계안 §4·network.md·README 갱신) ② **설계안 2단계 실행**: 스텝메이드 브랜치 "Claude Test"에 `🌏 마스터 파일` 페이지(`3420:6`, 루트 `3420:91`)를 지침서 §6 규격대로 만들고 Δ 화면 11장 + 케이스 표 3장 + 진입 섹션(SVOD·TVOD 홈 렌더 이미지 + 마커 3개) + 링크 카드까지 채움. 본 파일 무수정. 기록 `docs/demo-2026-09-10-stepmade.md` §8, 스크린샷 `docs/img/stepmade-core-*-2026-09-11.png` 5장 ③ 지침서 **v0.2**(§4-2 규칙 6줄·§4-3 마커 신설·§6-2 수치와 컴포넌트 키·진입 섹션 렌더 이미지 규칙·§6-4 방법 2) + 절차 문서 `scripts/figma-write/new-core-master.md`
- **세션 5 후반 (2026-09-11, Ken 교정 4건 + 프로그램화 1단계)**: ① 헤더 케이스 표가 옆 묶음과 겹침 → 실제 자식 폭을 재서 재배치 ② 🔵 크기 바에 폭 범위 설명(APP ~599/600~799/800~, WEB ~767/768~1280/1281~) ③ 복사본이 원본과 다름(셀 폭 183→153 등) → **B' 원본 대조** 단계를 모든 복제에 필수로 ④ 데스크톱 1440 누락 → WEB large=1440, 1280은 medium 구간으로. **두 원칙을 §0-6·§0-7로 규칙화**: "새로 만들지 말고 복붙" + "빠뜨리지 않기(수록 대조표)". ⑤ **프로그램화 1단계**: 실행 규약 `docs/runbook-core-helper.md`(Notion 표 상태 흐름·쓰기 범위·사람 손), Routine 프롬프트 `docs/routine-prompt-core-helper.txt`. 세션에서 만든 Routine은 연결을 못 담아 꺼 둠 → **Ken이 Routines 화면에서 직접 생성**(규약 §4). 유형 A/B 실행은 3단계 검증 전이라 제안서까지만
- **Ken 컨펌 대기 (브랜치, 되돌리기 쉬움)**: 묶음 A 뼈대 / B APP / C WEB / D 진입 각각의 스크린샷(§8 표) + 판단 10개(§8-1). 특히 ⓐ 진입 화면을 렌더 이미지로 둔 것 ⓑ 마커를 단순 프레임으로 둔 것(영역 설명 컴포넌트가 이 파일에서 import 안 됨 → 라이브러리 연결 후 교체 여부) ⓒ 로딩을 표로 바꾼 것(Δ #6 정정)
- **여전히 못 받은 것**: 과거 이관 사례 1건(프로젝트 문서 링크 + 반영된 Core 위치) → 받으면 §3·§4 재검증 → v0.3. 세션 5 끝에 한 번 더 물어봄
- **다음 할 일**: ⓐ Ken이 Routine 생성(규약 §4) → 표에 시험용 행 1개 넣어 첫 자동 실행 확인 ⓑ 설계안 3단계: 기존 Core 갱신(유형 A/B) 실행 검증(로그인/온보딩 파일럿 재현) ⓒ (승인 후) 스텝메이드 본 파일 쓰기 단계 — Cover 텍스트·업데이트 로그, `🌊 플로우` 뼈대, SVOD·TVOD·검색 플로우 입구 Δ #13~15, 파일 이름·폴더 이동(사람), 색인 갱신. 그다음 설계안 3단계(기존 Core 갱신 자동화: 로그인/온보딩 파일럿 재현) 또는 4단계(Notion 요청 표 + Routine)
- 기술: Figma 연결(MCP)이 세션 중 여러 번 끊겼다 붙음 → 도구 접두어가 `mcp__ede8b140…` → `mcp__Figma__`로 바뀜. ToolSearch로 다시 찾으면 되고 브랜치에 쓴 것은 남아 있음. Ken이 "다시 시도"를 보내면 그 시점부터 이어 감
...
- 스크린 설명 컴포넌트 인스턴스의 속성 키: `🪐 뱃지#2018:10` / `🔠 타이틀#2018:9` / `ㄴ 📝 설명#2018:8` (불리언) / `배경 색상` (`⚫️ 어두운 회색` = 파일·대분류 바, `⚪️ 밝은 회색` = 케이스 묶음 바, `🔵 파랑` = 크기·단계 바). 컴포넌트 키(세션 5): ⚫ `47bbf037…` / ⚪ `1ea0bc15…` / 🔵 `94dc30fc…`, 세트 `022c72c2…`; 상태 뱃지 `11bae7fa…`; 영역 설명 컴포넌트 `73a99eba…`(세트 `4654fb2f…`, 스텝메이드 파일에선 import 불가). 전체 키는 지침서 §6-2
- 이미지 이관(세션 5 확정): `download_assets` → curl → `upload_assets(nodeIds)` → `curl -F` → 응답 해시로 fills 확인/지정. 절차 `scripts/figma-write/deep-clone.md` 3단계
