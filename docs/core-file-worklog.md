...
- **프로젝트의 목적 재확인 (Ken, 2026-09-10)**: "지금 바로 적용"이 아니라 **다른 디자이너들도 쓰는 프로그램**을 만들어 Core 관리의 통일성을 높이고 어려움·귀찮음을 줄이는 것. → **설계안 v1 작성**: `docs/plan-core-helper-plugin.md` (아티팩트 "Core 도우미 설계안"). 형태 = Figma 플러그인 + 규칙 엔진 + 애매할 때만 AI. 기능 4개 A 새 Core 만들기(완전 자동) / B Δ 만들기(반자동) / C 이관하기(반자동, 붙여넣기만 사람) / D 점검하기(완전 자동). 단계: 1단계 A+D → 2단계 C → 3단계 B+AI → 4단계 팀 배포. **Ken 결정 대기**: 이 설계안 승인, 1단계 두 기능의 순서, 플러그인 이름·말투. 지금까지 만든 색인·지침서·스크립트는 규칙 엔진의 재료
