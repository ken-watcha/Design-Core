...
- **새 형식**: 🌏 마스터 파일 페이지(대표 화면만) + 🌊 플로우 페이지(기준 문서 역할, 전수). 마스터 페이지 = 루트 섹션 → [📎 스펙 링크 카드] + APP 섹션 + WEB 섹션. 설명 바 색: ⚫️ 어두운 회색=대분류, ⚪️ 밝은 회색=케이스 묶음, 🔵 파랑=크기·단계
- **구 형식**: 스텝메이드: 크기별 섹션 안에 Guide/예시/뷰포트 폭 프레임. 마스터 페이지 없음, 새 수록 기준 미적용
