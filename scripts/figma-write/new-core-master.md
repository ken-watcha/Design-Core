...
| **B' 원본 대조(필수)** | 복제 직후 원본과 복사본의 트리를 같은 순서로 걸으며 크기·위치가 다른 노드를 원본 값으로 되돌린다(`resize`, 오토레이아웃 밖이면 `x/y`). `clone()`만으로는 **같은 파일 안에서도 셀 폭이 바뀌는 경우가 있다**(스텝메이드 768·1024: 포스터 셀 183→153, 그리드 로우 964→814 — 셀이 HUG라 내용 폭이 다시 계산됨). 남은 차이 0을 확인한 뒤 다음 묶음으로 | Ken 원칙: "새로 짜는 게 아니라 작업된 프레임을 그대로 복사해 우리 기준으로 분류" — 복사본은 원본과 픽셀 단위로 같아야 한다 |
