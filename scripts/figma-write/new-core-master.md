...
두 원칙 (Ken, 2026-09-11 — 지침서 §0-6·§0-7):
- **복붙**: 화면은 프로젝트 문서의 프레임을 `clone()`으로 그대로. 도우미가 만드는 것은 섹션·설명 바·케이스 표 껍데기·링크 카드·마커뿐. 복제마다 B' 원본 대조 → 남은 차이 0 확인.
- **빠뜨리지 않기**: 시작 전에 문서의 섹션 목록을 읽어(`figma.root.children` → 페이지 → SECTION 이름·안의 프레임 폭) 수록 대조표를 만들고, 끝난 뒤 표의 모든 행이 마스터에 있거나 "미수록 이유"가 적혀 있는지 센다.
